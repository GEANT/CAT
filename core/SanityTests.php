...
class SanityTests extends CAT
{
    ...
    private function getExecPath($pathToCheck)
    {
        ...
        foreach ([\config\Main::PATHS, \config\ConfAssistant::PATHS, \config\Diagnostics::PATHS] as $config) {
    ...
    {
        $rootUrl = substr(\config\Main::PATHS['cat_base_url'], -1) === '/' ? substr(\config\Main::PATHS['cat_base_url'], 0, -1) : \config\Main::PATHS['cat_base_url'];
    ...
    {
        if (!is_file(\config\Main::AUTHENTICATION['ssp-path-to-autoloader'])) {
            $this->storeTestResult(\core\common\Entity::L_ERROR, "<strong>simpleSAMLphp</strong> not found!");
        } else {
            include_once \config\Main::AUTHENTICATION['ssp-path-to-autoloader'];
    ...
    private function testSecurity()
    {
        if (in_array("I do not care about security!", \config\Main::SUPERADMINS)) {
            $this->storeTestResult(\core\common\Entity::L_WARN, "You do not care about security. This page should be made accessible to the CAT admin only! See config-main.php: 'SUPERADMINS'!");
        }
    }
    ...
    private function testGeoip()
    {
        ...
        switch (\config\Main::GEOIP['version']) {
            case 0:
            ...
            case 1:
                ...
                break;
            case 2:
                if (!is_file(\config\Main::GEOIP['geoip2-path-to-autoloader'])) {
                ...
                if (!is_file(\config\Main::GEOIP['geoip2-path-to-db'])) {
                ...
                include_once \config\Main::GEOIP['geoip2-path-to-autoloader'];
                $reader = new Reader(\config\Main::GEOIP['geoip2-path-to-db']);
                try {
                ...
                break;
            default:
                $this->storeTestResult(\core\common\Entity::L_ERROR, 'Check \config\Main::GEOIP[\'version\'], it must be set to either 1 or 2');
                break;
        }
    }
    ...
    private function testDatabases()
    {
        ...
        if (!empty(\config\Main::DB[$databaseName3])) {
            try {
    ...
    {
        if (empty(\config\Main::APPEARANCE['abuse-mail']) || \config\Main::APPEARANCE['abuse-mail'] == "my-abuse-contact@your-cat-installation.example") {
        ...
        $mail->SMTPAuth = true;
        $mail->SMTPSecure = 'tls';
        $mail->Host = \config\Main::MAILSETTINGS['host'];
        $mail->Username = \config\Main::MAILSETTINGS['user'];
        $mail->Password = \config\Main::MAILSETTINGS['pass'];
        $mail->SMTPOptions = \config\Main::MAILSETTINGS['options'];
        $mail->WordWrap = 72;
        $mail->isHTML(FALSE);
        $mail->CharSet = 'UTF-8';
