...
// use PHPMailer to send the mail
        ...
        if (\config\Main::MAILSETTINGS['user'] === NULL && \config\Main::MAILSETTINGS['pass'] === NULL) {
            $mail->SMTPAuth = false;
        } else {
            $mail->SMTPAuth = true;
...
// are we fancy? i.e. S/MIME signing?
        if (isset(\config\Main::MAILSETTINGS['certfilename'], \config\Main::MAILSETTINGS['keyfilename'], \config\Main::MAILSETTINGS['keypass'])) {
            $mail->sign(\config\Main::MAILSETTINGS['certfilename'], \config\Main::MAILSETTINGS['keyfilename'], \config\Main::MAILSETTINGS['keypass']);
        }
        return $mail;
    }
...
// what do we want to say?
        Entity::intoThePotatoes();
        $mail->Subject = sprintf(_("%s: you have been invited to manage an %s"), \config\Main::APPEARANCE['productname'], Entity::$nomenclature_participant);
        Entity::outOfThePotatoes();
        $mail->Body = $message;
