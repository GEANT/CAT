...
// return PKCS#12 data stream
    ...
    public static function triggerNewOCSPStatement($serial) {
        ...
        if (CONFIG['CONSORTIUM']['silverbullet_CA']['type'] != "embedded") {
            ...
            $execCmd = CONFIG['PATHS']['openssl'] . " ocsp -issuer " . ROOT . "/config/SilverbulletClientCerts/real.pem -sha256 -sha256 -ndays 10 -no_nonce -serial 0x$serialHex -CA " . ROOT . "/config/SilverbulletClientCerts/real.pem -ndays 365 -rsigner " . ROOT . "/config/SilverbulletClientCerts/real.pem -rkey " . ROOT . "/config/SilverbulletClientCerts/real.key -index $tempdir/index.txt -no_cert_verify -respout $tempdir/$serialHex.response.der";
