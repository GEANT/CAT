...
<html xmlns="http://www.w3.org/1999/xhtml" lang="<?php echo $langObject->getLang() ?>">
    <head lang="<?php echo $langObject->getLang() ?>"> 
        <title><?php echo \config\Main::APPEARANCE['productname_long']; ?></title>
    ...
    <body style='background: #fff url(resources/images/bg_grey_tile.png) repeat-x;'>
        ...
        <div id="motd"><?php print ( isset(\config\Main::APPEARANCE['MOTD']) ? \config\Main::APPEARANCE['MOTD'] : '&nbsp'); ?></div>

        <h1><a href="<?php echo dirname($_SERVER['SCRIPT_NAME']) . '?lang=' . $langObject->getLang(); ?>"><?php echo \config\Main::APPEARANCE['productname']; ?></a></h1>
        <div id="tou">
            <?php
            ...
            <table style='width:100%'>
                ...
                <tr>
                    ...
                    if (!empty(\config\Main::APPEARANCE['privacy_notice_url'])) {
                        $retval .= "<td><a href='".\config\Main::APPEARANCE['privacy_notice_url']."'>" . sprintf(_("%s Privacy Notice"),\config\ConfAssistant::CONSORTIUM['name']) . "</a></td>";
                    }
                    ?>
