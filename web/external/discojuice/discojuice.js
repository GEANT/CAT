...
DiscoJuice.UI = {
    ...
    "addItem": function (item, countrydef, search, distance) {
        ...
        var iconpath = this.parent.Utils.options.get('iconPath', '');
        ...
        if (countrydef || (distance != undefined)) {
            ...
            if (distance != undefined) {
                if (distance < 1) {
                    textLink += '<span class="distance">' + this.parent.Utils.options.get('textNearby', 'Nearby') + '</span>';
                } else {
    ...
    "refreshData": function (showmore, show, listcount) {
        ...
        if (showmore) {
            var textLimited1 = this.parent.Utils.options.get('textLimited1', 'Results limited to');
            var textLimited2 = this.parent.Utils.options.get('textLimited2', 'entries – show more');
            var moreLink = '<a class="discojuice_showmore textlink" href="">' + textLimited1 + ' ' + show + ' ' + textLimited2 + '…</a>';
    ...
    "enable": function (control) {
        ...
        if (this.parent.Utils.options.get('location', false) && navigator.geolocation) {
            ...
            $("a#locateme").click(function (event) {
                ...
                $("div.locatemeafter").html('<div class="loadingData" ><img src="' + imgpath + 'spinning.gif" /> ' + that.parent.Utils.options.get('geoLoc_getting', 'Getting your location...') + '</div>');
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                    function (position) {
                        ...
                        that.ui.setLocationText(that.parent.Utils.options.get('geoLoc_here', 'You are here:') + ' ' + position.coords.latitude + ', ' + position.coords.longitude + '. ' + that.parent.Utils.options.get('geoLoc_nearby', 'Nearby providers shown on top.'));
                    ...
                    // next function is the error callback
                            function (error) {
                                switch (error.code) {
                                    case error.TIMEOUT:
                                        that.ui.setLocationText(that.parent.Utils.options.get('geoLoc_timeout', 'Timeout'));
                                        break;
                                    case error.POSITION_UNAVAILABLE:
                                        that.ui.setLocationText(that.parent.Utils.options.get('geoLoc_posUnavailable', 'Position unavailable'));
                                        break;
                                    case error.PERMISSION_DENIED:
                                        that.ui.setLocationText(that.parent.Utils.options.get('geoLoc_permDenied', 'Permission denied'));
                                        break;
                                    case error.UNKNOWN_ERROR:
                                        that.ui.setLocationText(that.parent.Utils.options.get('geoLoc_unknownError', 'Unknown error'));
                                        break;
                                }
                            },
                            {maximumAge: 600000}
...
//				that.resetCategories();							
    ...
    "filterCountrySetup": function (choice) {
        ...
        var textShowProviders = this.parent.Utils.options.get('textShowProviders', 'Show providers in');
        var textAllCountries = this.parent.Utils.options.get('textAllCountries', 'all countries');
        var textShowAllCountries = this.parent.Utils.options.get('textShowAllCountries', 'show all countries');
        var ftext = '<p class="discojuice_filter_country">' + textShowProviders + ' ' +
                '<select class="discojuice_filterCountrySelect" name="filterCountrySelect">';

        if (choice) {
            ftext += '<option value="all">' + textAllCountries + '</option>';
        } else {
            ftext += '<option value="all" selected="selected">' + textAllCountries + '</option>';
        }
        ...
        ftext += '</select>';
        ftext += ' <a class="discojuice_showall textlink" href="">' + textShowAllCountries + '</a>';
        ftext += '</p>';
    ...
    "getCountry": function () {
        ...
        if (countryapi) {
            this.parent.Utils.log(countryapi);
            ...
            if (countrycache) {
                ...
                this.parent.Utils.log('jest cache');
...
//alert('W');
                this.parent.Utils.log('brak cache');
