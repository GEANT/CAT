...
echo $deco->defaultPagePrelude(sprintf(_("%s: %s Dashboard"), \config\Main::APPEARANCE['productname'], $uiElements->nomenclatureInst));
            ...
            case "core\ProfileRADIUS":
                ...
                if (\config\Main::FUNCTIONALITY_LOCATIONS['DIAGNOSTICS'] !== NULL) {
                    if (\config\Main::FUNCTIONALITY_LOCATIONS['DIAGNOSTICS'] == "LOCAL") {
                        ...
                        $diagUrl = \config\Main::FUNCTIONALITY_LOCATIONS['DIAGNOSTICS'] . "/diag/";
                    }
...
// dummy width to keep a little distance
    ...
    if ($readonly === FALSE) {
        ...
        if (\config\Main::FUNCTIONALITY_LOCATIONS['CONFASSISTANT_SILVERBULLET'] == "LOCAL" && count($myfed->getAttributes("fed:silverbullet")) > 0 && $sbProfileExists === FALSE) {
