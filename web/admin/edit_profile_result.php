...
if ((int) $_POST['submitbutton'] === web\lib\common\FormElements::BUTTON_DELETE) {
...
if ((int) $_POST['submitbutton'] === web\lib\common\FormElements::BUTTON_SAVE) {
...
// extended input checks
    ...
    foreach ($_POST as $key => $value) {
        switch ($key) {
            case 'anon_support':
            case 'checkuser_support':
            case 'verify_support':   
            case 'hint_support':
            case 'redirect':    
                $$key = $validator->boolean($value);
                break;
            case 'checkuser_local':
            case 'anon_local':
                $$key = $validator->string($value);
                break;
            case 'realm':
                $realm = $validator->realm(htmlspecialchars(strip_tags($value)));
                break;
            default:
                break;
        }
    ...
    <table>
        ...
        if ($anon_support !== FALSE) {
            ...
            if ($verify_support === FALSE) { // no anon outer ID, and no realm suffix verification? Bad idea!
        ...
        if ($verify_support !== FALSE) {
            $profile->setInputVerificationPreference($verify_support, $hint_support);
            $extratext = "";
            if (!empty($realm)) {
                if ($hint_support !== FALSE) {
