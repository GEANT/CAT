...
if (count($my_inst->listProfiles()) == 0) {
    ...
    if (CONFIG['FUNCTIONALITY_LOCATIONS']['CONFASSISTANT_SILVERBULLET'] == "LOCAL" && count($allow_sb) > 0) {
        ...
        if (count($my_inst->getAttributes("support:email")) > 0) {
            ...
            echo "<table>";
            echo $uiElements->boxError(sprintf(_("You did not submit an e-mail address. This is required for %s. Please go to the %s dashboard and edit your %s settings to include a helpdesk e-mail address."), core\ProfileSilverbullet::PRODUCTNAME, $ui->nomenclature_inst, $ui->nomenclature_inst), _("No support e-mail!"));
            echo "</table>";
        }
    }
