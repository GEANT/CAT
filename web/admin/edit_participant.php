...
if ($wizardStyle) {
    echo $deco->defaultPagePrelude(sprintf(_("%s: %s enrollment wizard (step 2)"), \config\Main::APPEARANCE['productname'],  $uiElements->nomenclatureParticipant));
} else {
    echo $deco->defaultPagePrelude(sprintf(_("%s: Editing %s '%s'"), \config\Main::APPEARANCE['productname'], $uiElements->nomenclatureParticipant, $inst_name));
}
