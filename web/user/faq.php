...
array_push($FAQ, [
    ...
    'text' => sprintf(_("Contact %s administrators within your organisation and request that they add their organisation to the system. It will take at most one hour of their time to get things done."), \config\ConfAssistant::CONFIG['CONSORTIUM']['display_name'])
]);

array_push($FAQ, [
    'title' => sprintf(_("My device is not listed! Does that mean I can't do %s?"), \config\ConfAssistant::CONFIG['CONSORTIUM']['display_name']),
    'text' => sprintf(_("No. The CAT tool can only support Operating Systems which can be automatically configured in some way. Many other devices can still be used with %s, but must be configured manually. Please contact your organisation to get help in setting up such a device."), \config\ConfAssistant::CONFIG['CONSORTIUM']['display_name'])
]);

array_push($FAQ, [
    'title' => sprintf(_("I can connect to %s simply by providing username and password, what is the point of using an installer?"), \config\ConfAssistant::CONFIG['CONSORTIUM']['display_name']),
...
if (\config\ConfAssistant::CONFIG['CONSORTIUM']['name'] == "eduroam") {
    array_push($FAQ, [
        'title' => sprintf(_("What is this %s thing anyway?"), \config\ConfAssistant::CONFIG['CONSORTIUM']['display_name']),
...
array_push($FAQ, [
    ...
    'text' => sprintf(_("%s installers configure security settings on your device, therefore you should be sure that you are using genuine ones."), \config\Master::CONFIG['APPEARANCE']['productname']) . ' ' . ( isset(\config\ConfAssistant::CONFIG['CONSORTIUM']['signer_name']) && \config\ConfAssistant::CONFIG['CONSORTIUM']['signer_name'] != "" ? sprintf(_("This is why %s installers are digitally signed by %s. Watch out for a system message confirming this."), \config\Master::CONFIG['APPEARANCE']['productname'], \config\ConfAssistant::CONFIG['CONSORTIUM']['signer_name']) : ""),
]);
...
array_push($FAQ, [
    ...
    'text' => _("Contrary to what the name suggests, 'SmartScreen' isn't actually very smart. The warning merely means that the file has not yet been downloaded by enough users to make Microsoft consider it popular (which would strangely enough make it be considered 'safe'). This message alone is not a security problem.") . " " . (isset(\config\ConfAssistant::CONFIG['CONSORTIUM']['signer_name']) && \config\ConfAssistant::CONFIG['CONSORTIUM']['signer_name'] != "" ? sprintf(_("So long as the file is carrying a valid signature from %s, the download is safe."), \config\ConfAssistant::CONFIG['CONSORTIUM']['signer_name']) . " " : "") . sprintf(_("Please see also Microsoft's FAQ regarding SmartScreen at %s."), "<a href='http://windows.microsoft.com/en-US/windows7/SmartScreen-Filter-frequently-asked-questions-IE9?SignedIn=1'>Microsoft FAQ</a>")
]);

if (\config\Master::CONFIG['FUNCTIONALITY_LOCATIONS']['DIAGNOSTICS'] == NULL) {
    $text = sprintf(_("There can be a number of different reasons. The network you see may not be a genuine %s one and your device silently drops the connection attempt; there may be something wrong with the configuration of the network; your account may have expired; there may be a connection problem with your home authentication server; you may have broken the regulations of the network you are using and have been refused access as a consequence. You should contact your organisation and report the problem, the administrators should be able to trace your connections."), \config\ConfAssistant::CONFIG['CONSORTIUM']['display_name']);
} else {
    if (\config\Master::CONFIG['FUNCTIONALITY_LOCATIONS']['DIAGNOSTICS'] == 'LOCAL') {
        $diagUrl = \config\Master::CONFIG['PATHS']['cat_base_url'] . "diag/";
    } else {
        $diagUrl = \config\Master::CONFIG['FUNCTIONALITY_LOCATIONS']['DIAGNOSTICS'];
    }
    $text = sprintf(_("There can be a number of different reasons. The network you see may not be a genuine %s one and your device silently drops the connection attempt; your account may have expired; you may have broken the regulations of the network you are using and have been refused access as a consequence; or maybe there is an actual technical problem. To exclude the latter, you should first visit the %s Diagnostics tool <a href=''>here</a>, which can find many reasons for failure. It also allows you to contact your organisation and report the problem to them."), \config\ConfAssistant::CONFIG['CONSORTIUM']['display_name'], $diagUrl);
}
array_push($FAQ, [
    'title' => sprintf(_("I can see %s network and my device is configured but it does not connect, what can be the cause?"), \config\ConfAssistant::CONFIG['CONSORTIUM']['display_name']),
    'text' => $text
]);
