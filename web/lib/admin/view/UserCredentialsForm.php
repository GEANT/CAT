...
class UserCredentialsForm implements PageElementInterface {
    ...
    private function addTitleRow() {
        $row = new Row ( array (
                ...
                self::TOKEN_COLUMN => 'Token/Certificate details',
                self::EXPIRY_COLUMN => 'User Expiry/Certificate Expiry',
                self::ACTION_COLUMN => 'Actions' 
        ) );
    ...
    public function addUserRow($user) {
        $row = new Row ( array (
                self::USER_COLUMN => $user->getUsername (),
                self::EXPIRY_COLUMN => new DatePicker ( SaveUsersCommand::PARAM_EXPIRY_MULTIPLE, $user->getExpiry () ) 
        ) );
    ...
    public function addCertificateRow($certificate) {
        if ($certificate->isGenerated ()) {
            ...
            if (! $certificate->isRevoked ()) {
                $row = new Row ( array (
                        'token' => $certificate->getCertificateDetails (),
...
<div class="<?php echo self::EDITABLEBLOCK_CLASS;?>">
        ...
        <div class="<?php echo self::ADDNEWUSER_CLASS; ?>">
                    ...
                    <label
                for="<?php echo AddUserCommand::PARAM_NAME; ?>"><?php echo _("Please enter a username of your choice and user expiry date to create a new user:"); ?></label>
            <div style="margin: 5px 0px 10px 0px;">
                <input type="text"
                    name="<?php echo AddUserCommand::PARAM_NAME; ?>">
                        <?php
        ...
        ?>
            ...
            <button type="submit" name="command"
                value="<?php echo AddUserCommand::COMMAND; ?>"><?php echo _('Add new user'); ?></button>
        </div>
    </form>
</div>
<?php
