...
class Authentication extends \core\common\Entity {
    ...
    public function isAuthenticated() {
        ...
        $authSimple = new \SimpleSAML\Auth\Simple(\config\Main::AUTHENTICATION['ssp-authsource']);
    ...
    public function authenticate() {
        ...
        if (!isset($admininfo[\config\Main::AUTHENTICATION['ssp-attrib-identifier']][0])) {
        ...
        $user = $admininfo[\config\Main::AUTHENTICATION['ssp-attrib-identifier']][0];

        $_SESSION['user'] = $user;
        $_SESSION['name'] = $admininfo[\config\Main::AUTHENTICATION['ssp-attrib-name']][0] ?? _("Unnamed User");
        /*
        ...
        foreach ($attribMapping as $SSPside => $CATside) {
            if (isset($admininfo[\config\Main::AUTHENTICATION[$SSPside]][0]) && (count($userObject->getAttributes($CATside)) == 0) && \config\Main::DB['USER']['readonly'] === FALSE) {
                $name = $admininfo[\config\Main::AUTHENTICATION[$SSPside]][0];
    ...
    public function deauthenticate() {
        ...
        $as = new \SimpleSAML\Auth\Simple(\config\Main::AUTHENTICATION['ssp-authsource']);
