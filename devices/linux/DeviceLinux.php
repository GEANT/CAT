...
class DeviceLinux extends \core\DeviceConfig {
    ...
    public function writeDeviceInfo() {
        ...
        $out .= sprintf(_("The installer is in the form of a Python script. It will try to configure %s under NetworkManager and if this is either not appropriate for your system or your version of NetworkManager is too old, a wpa_supplicant config file will be created instead."), \config\ConfAssistant::CONFIG['CONSORTIUM']['display_name']);
        $out .= "<p>";
        if ($ssidCount > 1) {
            if ($ssidCount > 2) {
                $out .= sprintf(_("In addition to <strong>%s</strong> the installer will also configure access to the following networks:"), implode(', ', \config\ConfAssistant::CONFIG['CONSORTIUM']['ssid'])) . " ";
            } else {
                $out .= sprintf(_("In addition to <strong>%s</strong> the installer will also configure access to:"), implode(', ', \config\ConfAssistant::CONFIG['CONSORTIUM']['ssid'])) . " ";
            }
            $iterator = 0;
            foreach ($this->attributes['internal:SSID'] as $ssid => $v) {
                if (!in_array($ssid, \config\ConfAssistant::CONFIG['CONSORTIUM']['ssid'])) {
                    if ($iterator > 0) {
                        $out .= ", ";
                    }
                    $iterator++;
