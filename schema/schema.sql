...
('eap:ca_file','certificate of the CA signing the RADIUS server key','file',NULL,1),
('eap:server_name','name of authorised RADIUS server','string',NULL,1),
('general:geo_coordinates','geographical coordinates of the institution or a campus','coordinates',NULL,1),
('general:instname','name of the institution in multiple languages','string','ML',0),
('general:instshortname','short name of the institution (acronym etc) in multiple languages','string','ML',0),
('general:instaltname','alternative name of the institution to be used in Disco search as keyword','string',NULL,0),
('general:logo_file','file data containing institution logo','file',NULL,0),
('media:SSID','additional SSID to configure, WPA2/AES only','string',NULL,0),
('media:wired','should wired interfaces be configured','boolean',NULL,0),
('media:remove_SSID','SSIDs to remove during installation','string',NULL,0),
('media:consortium_OI','Hotspot 2.0 consortium OIs to configure','string',NULL,0),
('media:force_proxy','URL of a mandatory content filter proxy','string',NULL,0),
('media:openroaming','enum switch to select desired OpenRoaming integration','enum_openroaming','VALUES:ask,always,ask-preagreed,always-preagreed',0),
('profile:name','The user-friendly name of this profile, in multiple languages','string','ML',1),
('profile:customsuffix','The filename suffix to use for the generated installers','string','ML',1),
('profile:description','extra text to describe the profile to end-users','text','ML',1),
('profile:production','Profile is ready and can be displayed on download page','boolean',NULL,0),
('hiddenprofile:tou_accepted','were the terms of use accepted?','boolean',NULL,0),
('support:email','email for users to contact for local instructions','string','ML',1),
('support:info_file','consent file displayed to the users','file','ML',1),
('support:phone','telephone number for users to contact for local instructions','string','ML',1),
('support:url','URL where the user will find local instructions','string','ML',1),
('user:email','email address of the user (from IdP)','string',NULL,0),
('user:fedadmin','contains federation names for which this user is an admin','string',NULL,0),
('user:realname','a friendly display name of the user','string',NULL,0),
('fed:realname','a friendly display name of the NRO/federation','string','ML',1),
('fed:url', 'URL to the homepage of a federation', 'string','ML',1),
...
('fed:silverbullet','enable Silver Bullet in this federation','boolean',NULL,0),
...
('fed:openroaming','Allow IdP OpenRoaming Opt-In','boolean',NULL,0),
('fed:openroaming_customtarget','custom NAPTR discovery target','string',NULL,0),
('fed:autoregister-synced', 'allow admins listed in eduroam DB to become admins for synced CAT institutions', 'boolean',NULL,0),
('fed:autoregister-new-inst', 'allow admins listed in eduroam DB to create new institutions', 'boolean',NULL,0),
('fed:autoregister-entitlement', 'allow entitlement and scope based addition of admins to CAT institutions', 'boolean',NULL,0),
('fed:entitlement-attr', 'the entitlement value used by this federation for self-registration, will override the default geant:eduroam:inst:admin','string',NULL,0),
('fed:max-inactivity', 'custom number of days of allowed admin inactivity', 'integer', NULL,0),
('fed:hide-admin-warnings', 'do not show any warnings about missing/inactive admins', 'boolean', NULL,0),
('fed:no-testing', 'stop periodic RADIUS testing', 'boolean', NULL,0),
('managedsp:vlan','VLAN tag to add if Managed IdP user logs into hotspot of organisation','integer',NULL,0),
('managedsp:realmforvlan','a realm which should get this VLAN tag, in addition to the Managed IdP ones (those are handled ex officio','string',NULL,1),
('managedsp:operatorname','Operator-Name attribute to be added to requests','string',NULL,0),
('hiddenmanagedsp:tou_accepted','were the terms of use accepted?','boolean',NULL,0),
('managedsp:name','hotspot display name', 'string',NULL,0);
