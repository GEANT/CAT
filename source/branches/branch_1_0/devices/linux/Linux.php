...
private function printStart() {
     ...
     if (Config::$CONSORTIUM['name'] == "eduroam")
         $out .= _("Installer created with software from the GEANT project.");
     else
         $out .= sprintf(_("This installer was brought to you by %s. See %s for details."),Config::$APPEARANCE['productname_long'],( isset($_SERVER['HTTPS']) ? 'https://' : 'http://') . $_SERVER['SERVER_NAME'] . dirname(dirname($_SERVER['SCRIPT_NAME'])));
     $out .= '"
         ';
  return $out;
}
