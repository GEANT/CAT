...
//     $out = sprintf(_("Currently this is a placeholder for ToU"));
     ...
     return;
  case 'develop':
     $out = sprintf(_("The most important need is adding new installer modules, which will configure particular devices.  CAT is makig this easy for you. If you know how to create an automatic installer then fitting it into CAT should be a piece of cake. You should start by contacting us at <a href='mailto:%s'>%s</a>, but please also take a look at <a href='%s'>CAT documentation</a>."),'cat-devel@geant.net','cat-devel@geant.net','doc/');
    break;
   case 'report':
$out = sprintf(_("If you are an end user experiencing a problem with the installation or usage of the network then please start by reporting it to your local IT support.<p> If you are a CAT network administrator or would like to report a problem or suggestion about the CAT system as a whole, then contact %s. The language there is English only." ),'<a href="'.Config::$APPEARANCE['support-url'].'">'.preg_replace('/^mailto::/','',Config::$APPEARANCE['support-url']).'</a>');
     break;
   case 'faq':
     include('faq.php');
     return;
