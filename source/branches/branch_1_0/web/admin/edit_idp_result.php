    ...
    if (isset($_POST['submitbutton'])) {
        if (( $_POST['submitbutton'] == BUTTON_SAVE || $_POST['submitbutton'] == BUTTON_CONTINUE) && isset($_POST['option']) && isset($_POST['value'])) { // here we go
            ...
            if (count($ssids) > 0) {
                ...
                echo UI_okay(sprintf(ngettext("Your installers will configure the following SSID: <strong>%s</strong>","Your installers will configure the following SSIDs: <strong>%s</strong>",count($ssids)), $printedlist), ngettext("SSID configured","SSIDs configured",count($ssids)));
            } else {
