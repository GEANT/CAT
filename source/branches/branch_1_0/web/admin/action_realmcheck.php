    ...
    <form method='post' action='inc/credentialcheck.php?inst_id=<?php echo $my_inst->identifier; ?>&amp;profile_id=<?php echo $my_profile->identifier; ?>' onsubmit='doCredentialCheck(this); return false;'>
        ...
        <fieldset class='option_container'>
            ...
            foreach (Config::$RADIUSTESTS['UDP-hosts'] as $host) {
                ...
                flush();
                $packetflow = array();
                $cmdline = Config::$PATHS['rad_eap_test']." -c -H " . $host['ip'] . " -P 1812 -S " . $host['secret'] . " -M 22:44:66:CA:20:00 $check_anon_id -u cat-connectivity-test@" . escapeshellarg($check_realm) . " -p nopassword -e TTLS -m WPA-EAP -t " . $host['timeout'] . " | grep 'RADIUS message:' | cut -d ' ' -f 3 | cut -d '=' -f 2";
                ...
                elseif (isset($packetcount[3]) && !isset($packetcount[11])) {
                    if ($check_thorough && $check_anon_id != "")
                        ...
                        echo UI_warning(_("<strong>Test FAILED</strong>: the request was rejected immediately, without EAP conversation. This is not necessarily an error: if the RADIUS server enforces that outer identities correspond to an existing username, then this result is expected. In all other cases, the server appears misconfigured.") . $measure);
