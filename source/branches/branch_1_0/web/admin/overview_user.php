...
<body>
    ...
    <div>
        ...
        if ($user->isFederationAdmin())
            echo '<a href="overview_federation.php"><button>' . _("Click here to manage your federations") . '</button></a>';
        ?>
    </div>
    <?php
    ...
    if (Config::$CONSORTIUM['name'] == 'eduroam')
        $helptext = "&nbsp;<h3 style='display:inline;'>" . sprintf(_("(Need help? Refer to the <a href='%s'>IdP administrator manual</a>)"),"https://confluence.terena.org/x/fgBYAg")."</h3>";
    else
        $helptext = "";
    if (sizeof($has_inst) > 0) {
        // we need to run the Federation constructor
        $unused = new Federation("LU");
        echo "<h2 style='display:inline-block;'>" . sprintf(ngettext("You are managing the following institution:", "You are managing the following <strong>%d</strong> institutions:", sizeof($has_inst)), sizeof($has_inst)) . "</h2>";
        echo $helptext;
        $instlist = array();
        $my_idps = array();
        $my_feds = array();
        ...
        echo "<h2 style='display:inline-block;'>" . _("You are not managing any institutions.") . "</h2>";
        echo $helptext;
    };
