...
<?php //print '<pre>'; print_r($_REQUEST); print_r($Gui->Args); print '</pre>'; ?>
        ...
        <div id="heading">
            <?php echo _("This is a service under preparation, do not expect it to work.") ?>
        </div>
        <?php
if($Gui->page == 0) {
       print "<h1 style='color:red'>"._("no maching data found")."</h1>";
       $Gui->page = 2;
}
...
print '<option value="0">'._("Select your institution").'</option>';
                ...
                foreach ($Inst as $I)
print '<option value="'.$I['entityID'].'">'.$I['title'].'</option>';
...
print $Gui->passArgument('lang');
            ...
            case 5:
                ...
                if($extra_text)
                  $extra_text .= '<p>';
                print $extra_text;
                print '<center><button style="width: 90%; height: 3em; font-size: 200%; cursor: pointer" onclick="window.location.href=\'' . rtrim(dirname($_SERVER['SCRIPT_NAME']),'/'). '/' . $o['link'] . '\'; return(false)">' . _("Download installer for") . '<br><span style="color:green; font-weight: bold">' . $D['display']. '</span></button></center>';
                $out = '';
                if (isset($a['local_email']) && $a['local_email'])
                    $out .= '<p>Email: <a href="mailto:' . $a['local_email'] . '">' . $a['local_email'] . '</a>';
                if (isset($a['local_url']) && $a['local_url'])
...
echo " &copy; 2011-12 DANTE Ltd. on behalf of the GN3 consortium</div>";?>
</body>
</html>
