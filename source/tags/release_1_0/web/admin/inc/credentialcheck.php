    ...
    foreach ($eaps as $pref => $eap) {
        foreach (Config::$RADIUSTESTS['UDP-hosts'] as $host) {
            if ($eap['OUTER'] == PEAP) {
                ...
                $cmdline = "rad_eap_test -c -H ".$host['ip']." -P 1812 -S ".$host['secret']." -M 22:44:66:CA:20:01 $anon_id -u ".$_POST['username']." -p ".$_POST['password']." -e PEAP -m WPA-EAP $certopts -t ".$host['timeout']." | grep 'RADIUS message:' | cut -d ' ' -f 3 | cut -d '=' -f 2";
                ...
                $cmdline = "rad_eap_test -c -H ".$host['ip']." -P 1812 -S ".$host['secret']." -M 22:44:66:CA:20:01 $anon_id -u ".$_POST['username']." -p ".$_POST['password']." -e TTLS -2 $inner_eap -m WPA-EAP $certopts -t ".$host['timeout']." | grep 'RADIUS message:' | cut -d ' ' -f 3 | cut -d '=' -f 2";
                $packetflow = array();
                exec($cmdline,$packetflow);
                ...
                $cmdline = "rad_eap_test -c -H ".$host['ip']." -P 1812 -S ".$host['secret']." -M 22:44:66:CA:20:01 $anon_id -e TLS -m WPA-EAP -t ".$host['timeout']." | grep 'RADIUS message:' | cut -d ' ' -f 3 | cut -d '=' -f 2";
                echo UI_incomplete("INCOMPLETE I would execute this:<br/>$cmdline","not implemented");
            }
            else
