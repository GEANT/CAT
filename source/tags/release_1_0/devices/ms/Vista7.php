...
//   writeWLANprofile ('eduroam','eduroam','WPA2','AES',$eap,$servers,$ca,$i)
...
fwrite($xml_f,$profile_file_contents.$vista_ext.$profile_file_contents_end);
fclose($xml_f);
$xml_f_name = "w7/wlan_prof-$i.xml";
$xml_f = fopen($xml_f_name,'w');
fwrite($xml_f,$profile_file_contents.$w7_ext.$profile_file_contents_end);
fclose($xml_f);
...
$sw2_f = fopen($f_name,'w');
 ...
 fwrite($sw2_f,$profile_file_contents);
fclose($sw2_f);
debug(2,"Installer has been written into directory $this->FPATH\n");
return($P);
}
...
$fcontents .= "\n".'!define USER_GROUP "'.$attr['profile:name'][0].'"';
$fcontents .= '
Caption "'. sprintf(_("%s installer for %s"),Config::$CONSORTIUM['name'],$this->translateString($attr['general:instname'][0], $this->code_page)).'"
!define APPLICATION "'. sprintf(_("%s installer for %s"),Config::$CONSORTIUM['name'],$this->translateString($attr['general:instname'][0], $this->code_page)).'"
!define VERSION "1.00"
...
debug(4,"code_page=".$this->code_page."\n");
   ...
   $result = $result && $this->copyFile('eduroam32.ico');
   $result = $result && $this->copyFile('eduroam_150.bmp');
...
/*
  we will be writing the following wlan_profiles:
   1. name=>eduroam, ssid=>eduroam, auth=>WPA2, encryption=>AES
   2. name=>(tkip_profile_name), ssid=>eduroam, auth=>WPA, encryption=>TKIP
   optionally
   3. name=>eduroam-local, ssid=>(additional_SSID), auth=>WPA2, encryption=>AES
   4. name=>(tkip_profile_name)-local, ssid=>(additional_SSID), auth=>WPA, encryption=>TKIP
