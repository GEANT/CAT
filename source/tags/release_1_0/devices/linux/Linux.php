...
';
     ...
     $installer_path = preg_replace('/ +/','_',$this->attributes['general:instname'][0]).'.sh';
   ...
   $out .= "<p>";
    if($ssid_ct > 1) {
        if($ssid_ct > 2) {
            $out .= _("In addition to <strong>eduroam</strong> the installer will also configure access to the following networks:")." ";
        } else
            $out .= _("In addition to <strong>eduroam</strong> the installer will also configure access to:")." ";
        $i = 0;
