...
debug(4,$profile);
        ...
        $AllAttributes = DBConnection::exec(Profile::$DB_TYPE, "SELECT option_name, option_value, device_id, eap_method.php_serialized as method, row 
                FROM profile_option 
                LEFT JOIN eap_method ON profile_option.eap_method_id = eap_method.eap_method_id 
        ...
        foreach ($temparray as $attrib) {
            if ($attrib["level"] == "Profile") {
                ...
                foreach ($this->priv_attributes as $approved_attrib)
                    if ($attrib["name"] == $approved_attrib["name"])
                        $ignore == "YES";
                if ($ignore != "YES")
                    $this->priv_attributes[] = $attrib;
            }
        ...
        $eap_m = DBConnection::exec(Profile::$DB_TYPE, "SELECT meth.php_serialized FROM supported_eap supp,eap_method meth
               WHERE supp.profile_id = $this->identifier AND supp.eap_method_id = meth.eap_method_id
               ORDER by preference");
        $returnarray = array();
        while ($eap = (mysqli_fetch_object($eap_m))) {
            $eaptype = unserialize($eap->php_serialized);
            $returnarray[] = $eaptype;
        }
    ...
    public function getUserDownloadStats($device = 0) {
        $returnarray = array();
        $numbers_q = DBConnection::exec(Profile::$DB_TYPE, "SELECT device_id, downloads_user FROM downloads WHERE profile_id = $this->identifier");
    ...
    public function addSupportedEapMethod($type, $preference) {
        $type_id = DBConnection::exec(Profile::$DB_TYPE, "SELECT eap_method_id from eap_method WHERE php_serialized = '" . serialize($type) . "'");
        if ($a = mysqli_fetch_object($type_id)) {
            DBConnection::exec(Profile::$DB_TYPE, "INSERT INTO supported_eap (profile_id, eap_method_id, preference) VALUES ("
                    . $this->identifier . ", "
                    . $a->eap_method_id . ", "
                    . $preference . ")");
        } else { // that EAP type is unknown
            exit(1);
        }
        $this->updateFreshness();
    }
    ...
    public function isEapTypeDefinitionComplete($eaptype) {
        ...
        if ($eaptype["OUTER"] == PEAP || $eaptype["OUTER"] == TLS || $eaptype["OUTER"] == TTLS || $eaptype["OUTER"] == FAST) {
            ...
            /* echo "<pre>";
              print_r($options);
              echo "</pre>"; */
            if (count($ca_option) > 0 && count($cn_option) > 0)
                return true;
...
//debug(4,"FLAG:$name:".$flags[$name]."\n");
    ...
    public function readyForShowtime() {
        ...
        // do we know at least one SSID to configure? If not, it's not ready...
        if (!isset($attribs['general:SSID']) && !isset($attribs['general:SSID_with_legacy']) && (!isset(Config::$CONSORTIUM['ssid']) || count(Config::$CONSORTIUM['ssid']) == 0) )
                $proper_config = FALSE;
    ...
    public function prepShowtime() {
        $proper_config = FALSE;
        $attribs = $this->getCollapsedAttributes();
        // admin has the ultimate say: if he doesn't want to go live, no further checks are needed
        if (isset($attribs['profile:production']) && $attribs['profile:production'][0] == "on") {
